...
export default function Programme() {
  return (
    <section id="programme" className={styles.section} aria-labelledby="prog-title">
      <div className={styles.inner}>
        <ScrollReveal>
          <header className={styles.sectionHeader}>
            <p className={styles.eyebrow}>Evening Programme</p>
            <h2 id="prog-title" className={styles.sectionTitle}>The Concert</h2>
            <div className={styles.sectionRule} />
          </header>
        </ScrollReveal>

        <PartDivider label="Part One" />
        {partOne.map(piece => <PieceCard key={piece.id} piece={piece} />)}
        <Interlude />

        <PartDivider label="Part Two" />
        {partTwo.map(piece => <PieceCard key={piece.id} piece={piece} />)}
